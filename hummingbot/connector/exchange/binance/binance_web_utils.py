...
async def fetch_historical_candles(
    ...
    """
    Low-level method to retrieve candle ticks for a given trading pair using public BinanceAPI
    """
    throttler = throttler or create_throttler()
    api_factory = build_api_factory_without_time_synchronizer_pre_processor(throttler=throttler)
    rest_assistant = await api_factory.get_rest_assistant()
    candles = await rest_assistant.execute_request(
        url=public_rest_url(path_url=CONSTANTS.KLINES_PATH_URL, domain=domain),
        method=RESTMethod.GET,
        params=dict(
            symbol=trading_pair,
            interval=interval,
    ...
    return list(map(lambda candle: Candle(time=int(candle[0]),
