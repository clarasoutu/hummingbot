...
from hummingbot.core.data_type.common import Candle, OrderType, TradeType
...
class BinanceExchange(ExchangePyBase):
    ...
    def get_historical(self, trading_pair: str, hours: int) -> List[Candle]:
        ...
        if trading_pair not in self._candles:
            self.logger().warning(f"{trading_pair} historical data not yet ready...")
            return []
        self.logger().network(f"Retrieved {len(self._candles[trading_pair])} candles for {trading_pair}")
        return self._candles[trading_pair]
