...
def maker_trading_pair_prompt():
    exchange = rsi_bb_config_map.get("exchange").value
...
async def order_amount_prompt() -> str:
    trading_pair = rsi_bb_config_map["market"].value
...
def on_validated_price_source_exchange(value: str):
    if value is None:
        rsi_bb_config_map["price_source_market"].value = None


def order_size_prompt() -> str:
    quote_token = rsi_bb_config_map["markets"].value.split(",")[0].split("-")[1]
